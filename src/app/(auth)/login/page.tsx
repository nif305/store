  ...
  return (
    ...
    >
      <div className="grid min-h-screen lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <section className="relative flex min-h-screen items-center justify-center px-4 py-3 sm:px-5 sm:py-8 lg:px-8 xl:px-12">
          <div className="absolute inset-0 opacity-50">
            <div className="absolute right-6 top-6 h-28 w-28 rounded-full bg-[#016564]/6 blur-3xl" />
            <div className="absolute bottom-6 left-6 h-24 w-24 rounded-full bg-[#d0b284]/16 blur-3xl" />
          </div>

          <div className="relative z-10 w-full max-w-[480px]">
            {/* Logo — mobile only, compact */}
            <div className="mb-3 lg:hidden">
              <div className="rounded-[20px] bg-[linear-gradient(135deg,#015857_0%,#016564_50%,#0b7f7c_100%)] px-4 py-3">
                <div className="flex items-center justify-center rounded-[14px] border border-white/10 bg-white/5 py-3">
                  <img src="/nauss-gold-logo.png" alt={t('portal.naussLogoAlt')} className="h-12 w-auto object-contain" />
                </div>
              </div>
            </div>

            <div className="rounded-[24px] border border-white/80 bg-white/92 p-4 shadow-soft sm:rounded-[32px] sm:p-8">
              {/* Header */}
              <div className="mb-4 flex items-center justify-between sm:mb-7 sm:flex-col sm:text-center">
                <div className="flex items-center gap-2 sm:flex-col sm:gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#016564]/10 text-[#016564] sm:h-14 sm:w-14">
                  ...
                  <h1 className="text-[18px] font-bold text-[#0f1d3b] sm:text-4xl sm:font-normal">
                    {t('auth.loginTitle')}
                  </h1>
                </div>
                <LanguageToggle />
              ...
              <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
                <div className="min-w-0">
                  <label className="mb-1.5 block text-[13px] font-normal text-[#1d2640] sm:mb-2 sm:text-[14px]">
                    {t('auth.loginEmail')}
                  </label>
                  <input
                    type="email"
                    ...
                    className="h-[46px] w-full rounded-[14px] border border-[#d8dee1] bg-white px-4 text-[14px] font-normal text-[#1d2640] outline-none transition placeholder:text-[#9aa3b2] focus:border-[#016564] focus:ring-4 focus:ring-[#016564]/10 sm:h-[54px] sm:rounded-[18px] sm:text-[15px]"
                    required
                  />
                </div>
                ...
                <label className="flex items-center gap-2 text-[12px] text-[#5f687b] sm:text-[13px]">
                  <input
                    type="checkbox"
                ...
                {error ? (
                  <div className="rounded-[12px] border border-red-200 bg-red-50 px-3 py-2.5 text-[12px] leading-5 text-red-700 sm:rounded-[14px] sm:px-4 sm:py-3 sm:text-[13px]">
                    {error}
                  </div>
                ) : null}
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex h-[46px] w-full items-center justify-center rounded-[14px] bg-[#016564] px-4 text-[14px] font-semibold text-white shadow-[0_12px_24px_rgba(1,101,100,0.18)] transition hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-70 sm:h-[54px] sm:rounded-[18px] sm:text-[15px]"
                >
                  {loading ? t('auth.loginLoading') : t('auth.loginButton')}
                </button>
              </form>

              <div className="mt-3 text-center text-[12px] text-[#5f687b] sm:mt-5 sm:text-[14px]">
                {t('auth.requestAccountPrompt')}
                <Link href="/request-account" className="mx-1.5 font-semibold text-[#016564] hover:underline">
                  {t('auth.requestAccountLink')}
                </Link>
              </div>

              {/* Training Kit — hidden on small mobile, shown on sm+ */}
              <div className="mt-3 hidden sm:grid gap-3">
                <Link
                  href="/training-kit"
              ...
              {/* Training Kit compact — mobile only */}
              <Link href="/training-kit" className="mt-3 flex items-center justify-center gap-2 rounded-[14px] border border-[#d8e3e1] bg-[#f8fbfb] py-2.5 text-[12px] font-semibold text-[#016564] sm:hidden">
                <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4" stroke="currentColor" strokeWidth="1.8">
                  <path d="M4 7.5 12 3l8 4.5-8 4.5L4 7.5Z" /><path d="M4 7.5v8.8L12 21l8-4.7V7.5" />
                </svg>
                {language === 'en' ? 'Training Kit Assistant' : 'مساعد تجهيز الدورة'}
              </Link>
            </div>
          </div>
        </section>
