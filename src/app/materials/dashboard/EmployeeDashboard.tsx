...
function RequestStoreModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: (code: string) => void }) {
  ...
  const categories = useMemo(() => ['الكل', ...Array.from(new Set(items.map((i) => i.category).filter(Boolean)))], [items]);

  const visible = useMemo(() => {
    const q = query.toLowerCase();
    return items.filter((i) => {
      if (i.status === 'OUT_OF_STOCK' && i.availableQty === 0) return false;
      const catOk = category === 'الكل' || i.category === category;
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!purpose.trim()) { setError('حقل الغرض مطلوب'); return; }
    if (!cart.length) { setError('أضف مادة واحدة على الأقل'); return; }
    const missingDate = cart.find((r) => r.item.type === 'RETURNABLE' && !r.expectedReturnDate);
    if (missingDate) { setError(`حدد تاريخ الإرجاع لـ "${missingDate.item.name}"`); return; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#F9F9F9]" dir="rtl">
      {/* Modal Header */}
      <div className="flex items-center justify-between border-b border-[#DADBD9] bg-white px-5 py-3.5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[15px] font-extrabold text-[#2A2A2A]">طلب مواد جديد</div>
            <div className="text-[11px] text-[#B5BDBE]">{cart.length > 0 ? `${cart.length} صنف · ${totalUnits} وحدة` : 'اختر المواد المطلوبة'}</div>
          </div>
        </div>
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          <div className="border-b border-[#DADBD9] bg-white px-4 py-3 space-y-2">
            <div className="relative">
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="بحث عن مادة..."
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {loading ? (
              ...
              <div className="flex h-40 items-center justify-center text-[13px] text-[#B5BDBE]">لا توجد نتائج</div>
            ) : (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {visible.map((item) => {
                    ...
                    <article key={item.id}
                      ...
                      <div className="relative aspect-[4/3] overflow-hidden bg-[#F9F9F9]">
                        ...
                        <span className={`absolute bottom-1.5 right-1.5 rounded-full px-2 py-0.5 text-[9px] font-bold text-white ${avail === 0 ? 'bg-[#73384B]' : avail < 5 ? 'bg-[#6B5A4A]' : 'bg-[#4F8F7A]'}`}>
                          {avail > 0 ? `${avail} ${item.unit}` : 'نافد'}
                        </span>
                        {item.type === 'RETURNABLE' && (
                          <span className="absolute left-1.5 top-1.5 rounded-full bg-[#eef5f4] px-1.5 py-0.5 text-[9px] font-bold text-[#2A6364]">مسترجعة</span>
                        )}
                        {qty > 0 && (
        ...
        <form onSubmit={submit} className="flex w-[320px] shrink-0 flex-col border-r border-[#DADBD9] bg-white">
          <div className="border-b border-[#DADBD9] px-4 py-3">
            <div className="text-[14px] font-extrabold text-[#2A2A2A]">المواد المختارة</div>
          </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="mt-2 text-[12px] text-[#B5BDBE]">أضف مواد من القائمة</p>
              </div>
            ) : (
                  ...
                  <div key={row.item.id} className="rounded-[10px] border border-[#DADBD9] bg-[#F9F9F9] p-2.5">
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="text-[10px] text-[#B5BDBE]">{row.quantity} {row.item.unit} {row.item.type === 'RETURNABLE' ? '· مسترجعة' : '· مستهلكة'}</div>
                      </div>
                      <button type="button" onClick={() => setQty(row.item, 0)} className="text-[10px] text-[#73384B]">✕</button>
                    </div>
                    {row.item.type === 'RETURNABLE' && (
                      <div className="mt-2">
                        <label className="block text-[10px] font-semibold text-[#5A5A5A]">تاريخ الإرجاع المتوقع *</label>
          ...
          <div className="border-t border-[#DADBD9] px-4 pb-4 pt-3 space-y-3">
            {cart.length > 0 && (
              <div className="flex items-center justify-between rounded-[8px] bg-[#eef5f4] px-3 py-2 text-[12px]">
                <span className="text-[#2A6364]">{cart.length} صنف</span>
                <span className="font-bold text-[#2A6364]">{totalUnits} وحدة</span>
              </div>
            )}
            <div>
              <label className="block text-[11px] font-bold text-[#5A5A5A] mb-1">الغرض من الطلب *</label>
              <textarea value={purpose} onChange={(e) => setPurpose(e.target.value)} rows={3}
                placeholder="مثال: تجهيز قاعة تدريب أمن معلومات..."
            ...
            <button type="submit" disabled={submitting || cart.length === 0 || !purpose.trim()}
              ...
              {submitting ? (
                ...
                <><svg viewBox="0 0 24 24" fill="none" className="h-4 w-4" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z"/></svg>إرسال الطلب</>
              )}
            </button>
          </div>
        </form>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" dir="rtl">
      <div className="w-full max-w-[420px] overflow-hidden rounded-[20px] bg-white shadow-2xl">
        ...
        <div className="p-5 text-center">
          <p className="text-[14px] font-bold text-[#2A2A2A]">تم إرسال الطلب بنجاح</p>
          <p className="mt-1 text-[12px] text-[#B5BDBE]">سيتم مراجعته وصرفه من المستودع</p>
...
export function EmployeeDashboard({ metrics }: { metrics: Metrics | null }) {
  ...
  const stats = [
    { label: 'طلبات معلقة', value: metrics?.pendingRequests ?? 0, color: '#8a6a37', bg: '#f7f1e4', href: '/materials/requests', icon: (
    ...
    { label: 'طلبات مصروفة', value: metrics?.issuedRequests ?? 0, color: '#1e6b4c', bg: '#e8f5ef', href: '/materials/requests', icon: (
    ...
    { label: 'عهد نشطة', value: metrics?.activeCustody ?? 0, color: '#2A6364', bg: '#eef5f4', href: '/materials/custody', icon: (
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <section className="overflow-hidden rounded-[20px] bg-gradient-to-l from-[#1a3c3c] to-[#2A6364] p-5 text-white shadow-[0_16px_40px_rgba(42,99,100,0.3)]">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-[24px] font-extrabold">{user?.fullName?.split(' ').slice(0, 2).join(' ') || 'مرحباً'}</h1>
            <div className="mt-1 text-[12px] text-white/70">{user?.department || 'وكالة التدريب'}</div>
          </div>
          <button
      ...
      <section className="rounded-[20px] border border-[#DADBD9] bg-white p-5">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-[16px] font-extrabold text-[#2A2A2A]">متابعة طلباتي</h2>
          <a href="/materials/requests" className="text-[12px] font-semibold text-[#2A6364] hover:underline">عرض الكل</a>
        </div>
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            { label: 'بانتظار الصرف', value: metrics?.pendingRequests ?? 0, color: '#8a6a37', bg: '#f7f1e4', border: '#e8ddbf', href: '/materials/requests' },
            { label: 'مصروفة بيدي', value: metrics?.issuedRequests ?? 0, color: '#1e6b4c', bg: '#e8f5ef', border: '#cce6d7', href: '/materials/requests' },
            { label: 'طلبات مرتجعات', value: metrics?.returnRequestsTotal ?? 0, color: '#1b4f68', bg: '#e7eff5', border: '#b8d4e4', href: '/materials/returns' },
          ].map((item) => (
